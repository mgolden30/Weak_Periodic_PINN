...
def save_network_output( hydro_model, out_name, model_name ):
...
hydro_model = torch.load('network_output/hydro_model_4000.pth')
...
reset_adam_every = 1024

for epoch in range(epochs):
    if epoch % reset_adam_every == 0:
        #reintialize Adam to reset momentum
        optimizer = optim.Adam(hydro_model.parameters(), lr=0.005)
...
torch.save( hydro_model, 'final_trained_model.pth' )