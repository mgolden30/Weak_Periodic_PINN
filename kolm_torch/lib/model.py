...
class StreamfunctionNetwork(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Linear(  6,   L1 ) #sin and cos for 3 inputs gives 6 dimensions
        self.layer2 = nn.Linear(  L1,  L2 ) 
        self.layer3 = nn.Linear(  L2,  1 ) #single output is streamfunction
...
class HydroNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        T = self.T * torch.ones( w.size(), requires_grad=True )
        a = self.a * torch.ones( w.size(), requires_grad=True )
...
class WeakPINN(nn.Module):
    def __init__( self, hydro_model, nu, p ):
        ...
        Hx = D/8 #check weakform over an eigth of the domain
        Hy = D/8
        Ht = D/8
    ...
    def forward(self, xs, xs_uniform):
        ...
        displacement = torch.cat( ( self.H[0]/2*x[:,:,:,np.newaxis,np.newaxis],\
        ...
        eq1= - rt2 * rt * phi_dt * w \
             ...
             - phi * forcing
        err = torch.sum( weight * eq1, axis=[0,1,2] )
