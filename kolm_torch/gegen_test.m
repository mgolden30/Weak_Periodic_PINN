%{
Gegenbaur playground in Matlab to get an intuition for the quadrature
rules
%}


order = 16; %number of points
...
filename = "gegen_weights";
gegenbauer_rule ( order, alpha, a, b, filename )

alpha = 1/2;
filename = "leg_weights";
gegenbauer_rule ( order, alpha, a, b, filename )


%% Plot the resulting info
clf

w = readmatrix("gegen_weights_w.txt")
w = clean(w)

x = readmatrix("gegen_weights_x.txt")
x = clean(x)

scatter(x,w, 'filled');

hold on
w = readmatrix("leg_weights_w.txt")
w = clean(w)

x = readmatrix("leg_weights_x.txt")
x = clean(x)

scatter( x, w.*(1-x.^2).^2, 'filled');
...
function gegenbauer_rule ( order, alpha, a, b, filename )
